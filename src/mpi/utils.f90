...
contains
    ...
    subroutine read_inputfile(procid)
        ...
        if( ierr == 0 ) then
            ...
            ng(3) = nz    
            ! Print to screen
            if(procid == 0) print *, "*** Input file sucessfully read ***"
        else            
            if(procid == 0) error stop "ERROR: parameters.in file encountered a problem! :("
        end if
    ...
    subroutine setup_grid_spacing(procid,xin,yin,zin,nz_in,dx_out,dy_out,dz_out,dx_inv_out,dy_inv_out,dz_inv_out)
        !
        ...
        if(procid == 0) print * , "*** Sucessfully finished setting up the grid spacing ***"
    end subroutine setup_grid_spacing

    subroutine read_obj(procid,filename, vertices, normals, faces, face_normals, num_vertices, num_normals, num_faces)
        !
    ...
    subroutine tagminmax(procid,xin,yin,zin,bbox_min,bbox_max,nx_in,ny_in,nz_in,dx_in,dy_in,dz_in,sdfresolution,sx,ex,sy,ey,sz,ez)
        !
    ...
    subroutine compute_scalar_distance_face(procid,xstart,xend,ystart,yend,zstart,zend,xin,yin,zin,nfaces,input_faces,input_face_normals,input_vertices,input_normals,buffer_point_size,distance)
        !
        ...
        ! Output
        real(dp), intent(out) :: distance(xstart:xend,1:size(yin),1:size(zin))  ! Signed-Distance-Array
        ! Local variables
        ...
        integer :: printrank                                                    ! Rank that prints information        
        
        ! Set the central rank as an estimated heaviest portion
        printrank = nprocs/2
        
        ! Query the start time to add to the progress bar
        if(procid == printrank) call cpu_time(stime)                
    ...
    ! -- The subroutine below is not as rhobust as the flood-fill algorithm -- !
    ! subroutine fill_internal_simple(grid,sx, sy, sz, ex, ey, ez, large_negative_value)
    !     implicit none        
    !     integer, intent(in) :: sx, sy, sz, ex, ey, ez
    !     real(dp), intent(in) :: large_negative_value
    !     real(dp), intent(inout) :: grid(sx:ex,sy:ey,sz:ez)
    !     ! Local variables
    !     integer :: ii, jj, kk
    !     logical :: ispointinside
        
    !     ! Offset the start to allow for neighbor point
    !     do kk=sz+1,ez
    !         do jj=sy+1,ey
    !             ispointinside = .False.
    !             do ii=sx+1,ex
    !                 ! 
    !                 if(grid(ii,jj,kk) == scalarvalue .and. grid(ii-1,jj,kk) < 0.0_dp) then
    !                     ispointinside = .True.
    !                     grid(ii,jj,kk) = large_negative_value
    !                 else if(ispointinside .and. grid(ii,jj,kk) > 0.0_dp) then                        
    !                     grid(ii,jj,kk) = large_negative_value
    !                 else if(ispointinside) then
    !                     ispointinside = .False.                                            
    !                 end if
    !             end do
    !         end do
    ...
    ! end subroutine fill_internal_simple
    ...
    subroutine gather_array(proc_id, num_procs, local_sdf, global_nx, global_ny, global_nz, start_indices, end_indices, chunk_sizes, s_y, e_y, s_z, e_z, input_scalarvalue, assembled_array, ierr_mpi)
        !
        ! This subroutine gathers the array on processor 0 for I/O and other operations
        !
        use mpi
        implicit none
        ! Inputs
        ...
        ! Outputs
        real(dp), allocatable, intent(out), dimension(:,:,:) :: assembled_array
        integer, intent(out) :: ierr_mpi    
        ! Local variables
        real(dp), allocatable, dimension(:, :, :) :: recv_buffer
        integer :: rank
        integer, parameter :: mpi_datatype = MPI_DOUBLE_PRECISION
        integer, dimension(MPI_STATUS_SIZE) :: mpi_status
        real(dp) :: write_start_time, write_end_time
    
        if (proc_id == 0) then
            ! Allocate the global array to store the entire field
            allocate(assembled_array(global_nx, global_ny, global_nz))  
            ! Assign the default value for the array
            assembled_array = scalarvalue           
            ! Copy the data from rank 0 itself into the global array
            assembled_array(start_indices(0):end_indices(0), s_y:e_y, s_z:e_z) = local_sdf(:,s_y:e_y,s_z:e_z)
            ! Receive data from all other ranks
            do rank = 1, num_procs - 1
                ! Allocate buffer to receive the local sdf data from other ranks
                allocate(recv_buffer(chunk_sizes(rank), global_ny, global_nz))    
                ! Receive the data from rank ii
                call MPI_RECV(recv_buffer, chunk_sizes(rank) * global_ny * global_nz, mpi_datatype, rank, rank, MPI_COMM_WORLD, mpi_status, ierr_mpi)    
                ! Place the received data into the correct position in the global array
                assembled_array(start_indices(rank):end_indices(rank), :, :) = recv_buffer    
                ! Deallocate the temporary buffer
                deallocate(recv_buffer)
            end do                    
        else
            ! All other ranks send their local sdf data to rank 0
            call MPI_SEND(local_sdf, chunk_sizes(proc_id) * global_ny * global_nz, mpi_datatype, 0, proc_id, MPI_COMM_WORLD, ierr_mpi)
        end if
